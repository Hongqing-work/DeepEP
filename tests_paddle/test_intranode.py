...
def test_main(num_sms: int, local_rank: int, num_local_ranks: int, num_ranks: int, rank: int, buffer: deep_ep.Buffer, group: Group):
    # Settings
    ...
    topk_idx = paddle.topk(scores, num_topk, axis=-1, largest=True, sorted=False)[1]
    topk_weights = paddle.ones([num_tokens, num_topk], dtype=paddle.float32) * rank
    topk_weights_pure_rand = paddle.randn([num_tokens, num_topk], dtype=paddle.float32)
    ...
    for previous_mode in (False, True):
        for async_mode in (False, True):
            for current_x in (x_pure_rand, x, x_e4m3):
                ...
                paddle.distributed.alltoall_single(wsm_all2all_out, wsm_out, group=group, sync_op=False)
                paddle.base.core.nvprof_nvtx_pop()
    ...
    for current_x in (x_e4m3, x):
        ...
        if isinstance(current_x, tuple):
            ...
            best_dispatch_results = paddle.to_tensor([best_results[0], best_results[1]], dtype=paddle.int32)
...
def test_loop():
    ...
    buffer = deep_ep.Buffer(ep_group, int(1e9), num_rdma_bytes, low_latency_mode=test_ll_compatibility,
    ...
    for i in (24, ):
        test_main(i, local_rank, num_local_ranks, num_ranks, rank, buffer, ep_group)
        if local_rank == 0:
            print()

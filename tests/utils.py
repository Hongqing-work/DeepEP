...
def dump(x, name, local_rank):
    #print(f'wsm debug {name} {x}')
    #dtype2str = {torch.float32: "_orgi_fp32", torch.int: "_orgi_int", torch.int64: "_orgi_int64", torch.bool: "_orgi_bool", torch.bfloat16: "_orgi_bf16", torch.float8_e4m3fn: "_orgi_fp8"}
    name = "/root/paddlejob/workspace/env_run/liuyiqun/outputs/torch_dump/" + name
    print(name)
    if isinstance(x, torch.Tensor):
        if x.dtype==torch.float32 or x.dtype==torch.int or x.dtype==torch.int64 or x.dtype==torch.bool:
            y = x.cpu().numpy()
        elif x.dtype==torch.bfloat16:
            y = x.view(torch.uint16).cpu().numpy()
        elif x.dtype==torch.float8_e4m3fn:
            y = x.view(torch.uint8).cpu().numpy()
        else:
        ...
        assert y_scale.dtype==torch.float32
        y_dump = y.view(torch.uint8).cpu().numpy()
        y_scale_dump = y_scale.cpu().numpy()

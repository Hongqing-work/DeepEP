...
if [ "$1" = "bad_case" ]; then
    echo "export TEST_NCCL_ALLTOALL_EFFECT=1"
    export TEST_NCCL_ALLTOALL_EFFECT=1
else
    echo "run normal case without nccl all2all"
fi
...
export MASTER_ADDR="10.95.244.76" # 0
...
${nsys_args} python test_all2all_effect.py

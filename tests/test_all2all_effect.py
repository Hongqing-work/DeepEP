...
def run_one_all_to_all(group, size):
    input = torch.arange(size)
    output = torch.empty([size], dtype=torch.int64)
    dist.all_to_all_single(output, input, group=group)

def test_nccl_all2all_effect(num_local_ranks, num_nodes):
    flag = os.getenv('TEST_NCCL_ALLTOALL_EFFECT', '0')
    if flag == '1':
        size = num_local_ranks * num_nodes
        new_group = dist.new_group(list(range(size)))
        run_one_all_to_all(new_group, size)
        new_group.barrier()
        del new_group    
...
def test_main(local_rank: int, num_local_ranks: int, num_ranks: int, num_nodes: int, rank: int, group: dist.ProcessGroup, use_random_input, dump_input):
    ...
    for i in range(num_warmups + num_tests):
        ...
        if i >= num_warmups:
            ...
            if i == 500:
                test_nccl_all2all_effect(num_local_ranks, num_nodes)
    ...
    np.save("gpu_time.npy", gpu_times)
